...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:param name="lng_cost">Cost</xsl:param>
	<xsl:param name="lng_profit">Profit</xsl:param>
	...
	<xsl:param name="lng_task">Taks</xsl:param>
		...
		.invoice_tag {font-size: 12pt; font-weight: bold; align:}
 		...
 		<div class="report_postfix">
 			<div class="summary">
 				...
 				<div class="totalcost"><xsl:value-of select="$lng_cost" />: <xsl:value-of select="summary/@cost"/></div>
 				<div class="totalprofit"><xsl:value-of select="$lng_profit" />: <xsl:value-of select="summary/@profit"/></div>
 			</div>
 		</div>
 	...
 	<xsl:template match="invoice">
 		<div class="invoice">
 			...
 			<div class="summary">
	 			...
	 			<div class="totalcost"><xsl:value-of select="$lng_cost" />: <xsl:value-of select="totals/@cost"/></div>
 				<div class="totalprofit"><xsl:value-of select="$lng_profit" />: <xsl:value-of select="totals/@profit"/></div>
 			</div>
 			<hr></hr>
 		</div>
 	</xsl:template>	
 	...
 	<xsl:template match="task">
 		...
 		<table width="90%" border="0">
 			<tr>
 				...
 				<th width="12%"><xsl:value-of select="$lng_hours" /></th>
 				<th width="12%"><xsl:value-of select="$lng_rate" /></th>
 				<th width="12%"><xsl:value-of select="$lng_sum" /></th>
 				<th width="12%"><xsl:value-of select="$lng_discount" /></th>
 				<th width="12%"><xsl:value-of select="$lng_profit" /></th>
 			</tr>
 			...
 			<td></td>
 			<td class="summary_digit"><xsl:value-of select="summary/@profit"/></td>
 		</tr>
 		</table>
 	</xsl:template>
 	...
 	<xsl:template match="work">	
 		<tr>
 			<td colspan="7" class="workdescr"><xsl:value-of select="descr"/></td>
 		</tr> 		
 	...
 	<xsl:template match="charge">	
 		<tr>
 			...
 			<td class="digit"><xsl:value-of select="info/@profit"/></td>
 		</tr>
 	</xsl:template>
